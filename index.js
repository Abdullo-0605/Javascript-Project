  ...
  database.forEach(function (value, index) {
    databaseText += `
          <div class="column">
            ...
            <p class="title" id="title" onclick="editTitle(${index})">${value.title}</p>
...
function editTask(titleIndex, taskIndex) {
  ...
  // database[titleIndex].tasks.splice(taskIndex, 1, task);
}
...
function editTitle(titleIndex) {
  const editTitle = document.getElementById(`title-${titleIndex}`).index;
  database[titleIndex].title.splice(titleIndex, 1, editTitle);
  loopDatabase();
};